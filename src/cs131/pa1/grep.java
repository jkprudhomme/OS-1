...
public class grep extends SequentialFilter {
	String line;
	public void SequentialFilter(String line){
		this.line=line;
	}
	@Override
	public void process(){
		...
		if (parts.length <= 1) { // ( grep ) alone does not work
			...
			if (input.isEmpty()) {
				...
				/*
				 * separates into lines and find the line in the input that contains what we
				 * want
				 */
				while (!input.isEmpty()) {
			 ...
			 */
			/*
			 * cat display = new cat(); display.output = new LinkedList<String>();
			 * display.processLine(parts[2].trim()); while (!display.output.isEmpty()) {
			 * String[] found = display.output.poll().split("\r\n"); for (int i = 0; i <
			 * found.length; i++) { if (found[i].contains(parts[1].trim())) {
			 * output.add(found[i]); } } }
			 */
